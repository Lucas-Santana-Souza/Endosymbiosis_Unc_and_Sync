...
%% [growth rate of the Guest maximized first, s.t. (host's growth) > 0]
%   HOST          GUEST
...
% [   0   ] | [   S_int   ] [=] [0]                                           -> REGION b5  
% [   1    ] | [    0     ] [>] [0]                                           -> REGION for constrain in the growth rate when guest is maximized first, s.t. host's growth > 0

%% [growth rate of the Host maximized second, s.t. (guest max first)]
%   HOST          GUEST
% [ S_ext ] | [     0     ] [>] [(Host's rhs_ext_lb ) + (Guest's rhs_ext_lb )]-> REGION b1
% [ S_ext ] | [     0     ] [<] [(Host's rhs_ext_ub ) + (Guest's rhs_ext_ub )]-> REGION b2  
% [   0   ] | [S_unmapped ] [=] [0]                                           -> REGION b3  
% [ S_int ] | [ S_ext2int ] [=] [0]                                           -> REGION b4 
% [   0   ] | [   S_int   ] [=] [0]                                           -> REGION b5  
% [   0   ] | [     1     ] [>]  [result_pairij_step1_ofMGF_firstMj - ErrorTolerance;]   -> REGION  for constrain in the growth rate when host is maximized s.t. guest max first
...
ArraySize =  num_mat_files;                                            %CRITICAL PLACE TO RUN

%% Setting arrays that storage the max growth rate
gv_ancestral_alone_nonSharedEnv = zeros (ArraySize ,1);
gm_guestMaxFirst                = zeros (ArraySize ,ArraySize);
gm_host_st_guestMaxFirst        = zeros (ArraySize ,ArraySize);
...
parfor i = 1:ArraySize
    ...
    for j = 1:ArraySize
        ...
        %% 
        %  MAXIMIXING THE GUEST FIRST, THEN THE HOST
        % Create endo model in two steps
        
        % Step 1 of resultpair12: Maximize growth of guest (M2), given constrain that 
        %                         [growth of host (M1)] > 0  
        
        pairmodel = create_pair_step1_ofMGF_uei (ehmodel1 ,ehmodel2 ,ne ,ni ,nu);
        ...
        pairmodel = []; % Clearing the variable by overwriting with an empty array
        if result_pair12_step1_ofMGF_firstM2 > 0
            pairmodel = create_pair_step2_ofMGF_uei (ehmodel1 ,ehmodel2  ,ne ,ni ,nu ,result_pair12_step1_ofMGF_firstM2);
        ...
        gm_guestMaxFirst (i,j)         = result_pair12_step1_ofMGF_firstM2;
        gm_host_st_guestMaxFirst (i,j) = result_pair12_step2_ofMGF_firstM2;
...
if strcmp (dataUsed, '/ext_int_models_CarveMe')
    ...
    writematrix(gm_guestMaxFirst,                fullfile(SavingPathName, 'max_growth_G_st_growth_H_is_larger0_uei_CarveMe.csv'));
    writematrix(gm_host_st_guestMaxFirst,        fullfile(SavingPathName, 'max_growth_H_st_growth_G_is_maxFirst_uei_CarveMe.csv'));
elseif strcmp (dataUsed, '/ext_int_models_Agora')   
    ...
    writematrix(gm_guestMaxFirst,                fullfile(SavingPathName, 'max_growth_G_st_growth_H_is_larger0_uei_AGORA.csv'));
    writematrix(gm_host_st_guestMaxFirst,        fullfile(SavingPathName, 'max_growth_H_st_growth_G_is_maxFirst_uei_AGORA.csv'));
end
