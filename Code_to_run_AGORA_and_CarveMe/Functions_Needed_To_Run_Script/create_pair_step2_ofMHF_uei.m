...
%% Structure of the holobiont matrix (created in the 'create_pair_step2_ofMHF_uei.m' function):
%   HOST          GUEST
...
function endomodel = create_pair_step2_ofMHF_uei (ehmodel1 ,ehmodel2 ,ne ,ni, nu ,result_pairij_step1_ofMHF_firstMi)
...
f   = zeros(nrh+nre,1);
f(nrh + bmie) = -1; % guest's growth rate is maximized
endomodel.obj = f;
...
endomat(b4 ,nrh+1:nre+nrh) = ehmodel2.S_ext2int;

% REGION b5: c compartment endo
%endomat(b5 ,1:nrh) = 0;                
endomat(b5 ,nrh+1:nre+nrh) = ehmodel2.S_int;
...
endomodel.rhs(end) = result_pairij_step1_ofMHF_firstMi - ErrorTolerance; %This changes depending on who is max second or if the (Host's growth) = (Guest's growth)
