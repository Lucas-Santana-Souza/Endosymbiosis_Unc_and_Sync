...
%
%
function endomodel = create_pair_step2_ofMHF_cfe (ehmodel1 ,ehmodel2 ,ne ,nic ,nio ,nu ,result_pairij_step1_ofMHF_firstMi)
...
f   = zeros(nrh+nre,1);
f(nrh + bmie) = -1; % host's growth rate is maximized
endomodel.obj = f;
...
b8 = 1 + (2*nio +2*nic + 2*ne + nu);   % row indices corresponding to A for (host's growth) > [result_pairij_step1_ofMHF_firstMi - ErrorTolerance]
...
endomat(b8 ,bmih)     =  1;
%endomat(b8 ,bmie+nrh) = 0; % ensures they grow at same rate
...
endomodel.rhs(b8) = result_pairij_step1_ofMHF_firstMi - ErrorTolerance; %
...
endomodel.sense = [repmat('>' ,ne  ,1); ...  % -> REGION b1
                   ...
                   repmat('>' ,1   ,1)];     % -> REGION b8 This is '>' for the (Host's growth) > [result_pairij_step1_ofMHF_firstMi - ErrorTolerance]
