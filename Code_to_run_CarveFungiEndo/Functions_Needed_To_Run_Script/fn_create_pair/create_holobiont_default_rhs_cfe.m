...
%
%
function endomodel = create_holobiont_default_rhs_cfe (ehmodel1 ,ehmodel2 ,ne ,nic ,nio ,nu)
...
b8 = 1 + (2*nio +2*nic + 2*ne + nu);   % row indices corresponding to A for guest's growth = host's growth 
...
% REGION8 for (host's growth) = (guest's growth)
endomat(b8 ,bmih)     =  1;
...
endomodel.sense = [repmat('>' ,ne  ,1); ...  % -> REGION b1
                   ...
                   repmat('=' ,1   ,1)];     % -> REGION b8 This is '=' if the (Host's growth) = (Guest's growth)
