...
%
%
function endomodel = create_pair_step1_ofMGF_cfe_viab(ehmodel1 ,ehmodel2 ,ne ,nic ,nio ,nu  ,minGrowth)
...
b8 = 1 + (2*nio +2*nic + 2*ne + nu);   % row indices corresponding to A for host's growth > minGrowth
...
% REGION8 for  (host's growth) > minGrowth
endomat(b8 ,bmih)     =  1;
...
endomodel.sense = [repmat('>' ,ne  ,1); ...  % -> REGION b1
                   ...
                   repmat('>' ,1   ,1)];     % -> REGION b8 This is '>' for the (Host's growth) > minGrowth
