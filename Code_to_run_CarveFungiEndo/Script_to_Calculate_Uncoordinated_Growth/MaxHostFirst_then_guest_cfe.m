...
%      [wG2Endo]
%
%% Structure of 'create_pair_step1_ofMHF_cfe_viab(ehmodel1 ,ehmodel2 ,ne ,nic ,nio ,nu  ,minGrowth)':
%    HOST           GUEST
...
%
%% Structure of 'create_pair_step2_ofMHF_cfe (ehmodel1 ,ehmodel2 ,ne ,nic ,nio ,nu ,result_pairij_step1_ofMHF_firstMi)':
%    HOST           GUEST
% [  S_ext ] | [      0     ] [>] [(Host's rhs_ext_lb ) + (Guest's rhs_ext_lb )]-> REGION b1
% [  S_ext ] | [      0     ] [<] [(Host's rhs_ext_ub ) + (Guest's rhs_ext_ub )]-> REGION b2  
% [    0   ] | [ S_unmapped ] [=] [0]                                           -> REGION b3  
...
parfor i = 1:simulationSize
    ...
    gv_hostMaxFirst (i) = result_pair12_step1_ofMHF_firstM1;
    gv_guest_st_hostMaxFirst (i) = result_pair12_step2_ofMHF_firstM1;
...
SavingPathName = ['C:\Users\' username cloud_local '\growthResults_cfe\growthResultsTestH1G2Endo'] % P:= growth in sync
...
% Prefix used in saved name:
hostMaxFirst_prefix = ['v_rowID_hID_gID_wH1Endo_st_growth_G_is_larger' minGrowthStr '_cfe_default_rhs' ]
%guestMaxSec_prefix  = ['v_rowID_hID_gID_wG2Endo_cfe_default_rhs']
guestMaxSec_prefix  = ['v_rowID_hID_gID_wG2Endo_stV'  minGrowthStr '_cfe_default_rhs']
...
rowID_hID_gID_wH1Endo = [rowID_hID_gID_sampled(:, 1:3)   gv_hostMaxFirst];          % Changed here to only include the col 1, 2, & 3
rowID_hID_gID_wG2Endo = [rowID_hID_gID_sampled(:, 1:3)   gv_guest_st_hostMaxFirst]; % Changed here to only include the col 1, 2, & 3
...
% Saving files
writematrix(rowID_hID_gID_wH1Endo ,fullfile(SavingPathName ,[hostMaxFirst_prefix  '_Sampled'  num2str(simulationSize)  sufix_database '.csv' ]));
writematrix(rowID_hID_gID_wG2Endo ,fullfile(SavingPathName ,[guestMaxSec_prefix   '_Sampled'  num2str(simulationSize)  sufix_database '.csv' ]));
